...
class NfcService {
  ...
  /// type can be 'game', 'collectible', or 'shelf'
  /// data contains the game ID (int), collectible ID (int), or shelf letter (String)
        ...
        onDiscovered: (NfcTag tag) async {
          try {
            ...
            if (records.isNotEmpty) {
              ...
              // Check if it's a collectible tag (format: COLL:123)
              else if (text.startsWith('COLL:')) {
                final collectibleIdStr = text.substring(5); // Remove "COLL:" prefix
                final collectibleId = int.tryParse(collectibleIdStr);
                if (collectibleId != null) {
                  print('✅ Read collectible ID from tag: $collectibleId');
                  await NfcManager.instance.stopSession();
                  if (!completer.isCompleted) {
                    completer.complete({'type': 'collectible', 'data': collectibleId});
                  }
                } else {
                  print('❌ Invalid collectible tag format: $text');
  ...
  /// Write a collectible ID to an NFC tag
  Future<bool> writeCollectibleId(int collectibleId) async {
    final completer = Completer<bool>();
    var sessionActive = true;

    try {
      print('🏷️ Starting NFC write session for collectible ID: $collectibleId');
      print('🏷️ Please hold your iPhone to the NFC tag now...');
      ...
      await NfcManager.instance.startSession(
        ...
        onDiscovered: (NfcTag tag) async {
          ...
          try {
            ...
            // Create NDEF message with COLL: prefix
            final message = NdefMessage([
              NdefRecord.createText('COLL:$collectibleId'),
            ]);

            print('🏷️ Writing collectible tag...');

            // Write synchronously - this is critical
            try {
              await ndef.write(message);
              print('✅ Write successful!');
              NfcManager.instance.stopSession(alertMessage: 'Collectible tag written successfully!');
              if (!completer.isCompleted) completer.complete(true);
              sessionActive = false;
            } catch (writeError) {
              print('❌ Write error: $writeError');
